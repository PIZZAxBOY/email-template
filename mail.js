...
async function sendGmailApiMail(accessToken, mailOptions) {
...
async function sendMails(
  ...
  try {
    if (useGmailApi) {
      gmailAccessToken = await getAccessToken(smtpConfig?.auth?.user);
    } else {
    ...
    const message = useGmailApi ? "Gmail 授权失败" : "SMTP 连接失败";
    progress.stop(colors.red(message));
  ...
  try {
    for (let i = 0; i < recipients.length; i++) {
      ...
      try {
        if (useGmailApi) {
          await sendGmailApiMail(gmailAccessToken, mailOptions);
        } else {
...
async function searchSentMails(since) {
  ...
  try {
    if (choice.imap.host === "imap.gmail.com") {
      ...
      s.message("更新 token");
      const newToken = await getAccessToken(senderEmail);
